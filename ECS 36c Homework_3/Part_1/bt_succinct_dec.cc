...
void recur_tree(Node* n, std::list<int>& structure, std::list<int>& items) {
  ...
  if (structure.front()) {
    std::unique_ptr<Node> new_node_left(new BinaryTree<int>::Node{items.front()});
    items.pop_front();
  ...
  //  If structure list says 1, insert node
  if (structure.front()) {
    std::unique_ptr<Node> new_node_right(new BinaryTree<int>::Node{items.front()});
    items.pop_front();
...
bool check_items_input(const std::list<int>& items, const std::list<int>& structure) {
