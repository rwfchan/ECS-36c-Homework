...
class BST {
 ...
 private:
  ...
  /* Recursive functions for creating Stack and Queue */
  void stacking_tree(std::unique_ptr<Node>& n, std::stack<T>& tree_stack);
...
const T& BST<T>::floor(const T &key) {
  //  Create stack that stores items in b_tree, from the least to greatest
  std::stack<T> ordered_list;
  stacking_tree(root, ordered_list);
  ...
  //  Compare the top of stack to inputted key,
  //  If item is not what we need, pop it out of the stack
  while (ordered_list.top() > key) {
    ordered_list.pop();

    //  Error for invalid input to find floor, if input is too big
    if(ordered_list.empty()) {
...
const T& BST<T>::ceil(const T &key) {
  //  Use queue to store b_tree instead of stack
  //    This is to make finding the ceil easy
  ...
  //  Compare front of queue to inputted key
  //  If item is not what we need, pop it out of the queue
  while (ordered_list.front() < key) {
    ordered_list.pop();

    //  Error for invalid input to find ceil, if input is too small
    if(ordered_list.empty()) {
...
/*
 * Simple recur function to store trees into a stack from least to greatest
 *  Very similar to in-order traversing
 */
template <typename T>
void BST<T>::stacking_tree(std::unique_ptr<Node> &n, std::stack<T> &tree_stack) {
  if (!n) {
    return;
  }

  stacking_tree(n->left, tree_stack);
  tree_stack.push(n->key);
  stacking_tree(n->right, tree_stack);
}
 ...
 */
template <typename T>
void BST<T>::queueing_tree(std::unique_ptr<Node> &n, std::queue<T> &tree_queue) {
  if (!n) {
    return;
  }
